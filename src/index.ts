...
class Egnyte {
    ...
    private async _getCsrfToken(): Promise<string> {
        ...
        const csrfRegexp = query.match(
            /id='csrfmiddlewaretoken'.*value='([a-zA-Z0-9]+)'.*\n/
        );
        if (!csrfRegexp)
            throw new Error(
                "unable to find CSRF token in egnyte resellers login page"
            );
        return csrfRegexp[1];
    }
    ...
    private async _setResellerId(authCookie: string): Promise<string> {
        ...
        if (res.status === 302) {
            ...
            if (!location)
                throw new Error("unable to find location header in response");
            const resellerId = location.split("/")[5];
            this.resellerId = resellerId;
            return resellerId;
        } else {
            throw new Error(
                "an error occurred attempting to get the resellerId"
            );
        }
    }
    ...
    async _getAllPlanIds(authCookie: string): Promise<string[]> {
        ...
        const { data: res } = await this._egnyteRequest(
            `/msp/customer_data/${this.resellerId}`,
            {
                headers: { cookie: authCookie },
            }
        );
    ...
    private async _authenticate(): Promise<string> {
        ...
        const auth = await this._egnyteRequest(
            "/accounts/login/?next=/customer/browse/",
            {
                method: "post",
        ...
        if (auth.status === 302) {
            ...
            if (!setCookieHeader)
                throw new Error("unable to find set-cookie header in response");
    ...
    async getAllCustomers(): Promise<EgnyteCustomer[]> {
        ...
        for (const planId of planIds) {
            ...
            for (const customer of usageStatsRes.data) {
                const [customerEgnyteId, ref]: [string, any] =
                    Object.entries(customer)[0];
                ...
                const obj: EgnyteCustomer = {
                    ...
                    powerUsers: {
                        total:
                            ref.power_user_stats.Used +
                            ref.power_user_stats.Unused,
                    ...
                    storageGB: {
                        total:
                            ref.storage_stats.Used + ref.storage_stats.Unused,
    ...
    async getOneCustomer(customerId: string): Promise<EgnyteCustomer> {
        ...
        const customer = allCustomers.find(
            (customer) => customer.customerEgnyteId === customerId
        );
        if (!customer)
    ...
    async updateCustomer(customerId: string, data: UpdateCustomer) {
        ...
        if (!customer)
            throw new Error(`unable to find egnyte customer: ${customerId}`);
        if (
            data?.powerUsers?.total &&
            data.powerUsers.total !== customer.powerUsers.total
        ) {
            await this.updateCustomerPowerUsers(
                customerId,
                data.powerUsers.total
            );
            customer.powerUsers.total = data.powerUsers.total;
            customer.powerUsers.free =
                data.powerUsers.total - customer.powerUsers.used;
        }
        if (
            data?.storageGB?.total &&
            data.storageGB.total !== customer.storageGB.total
        ) {
            await this.updateCustomerStorage(customerId, data.storageGB.total);
            customer.storageGB.total = data.storageGB.total;
            customer.storageGB.free =
                data.storageGB.total - customer.storageGB.used;
        }
        return customer;
    }
        ...
        const results = await Promise.all(
            planIds.map(async (id) => {
                const [planDataRes, planPowerUserDataRes] = await Promise.all([
                    ...
                    this._egnyteRequest(
                        `/msp/get_plan_pu_data/${this.resellerId}/${id}/`,
                        {
                            headers: { cookie: authCookie },
                        }
                    ),
                ...
                return {
                    ...
                    usedPowerUsers:
                        planPowerUserData.purchased -
                        ref.power_user_stats.Available,
                    availablePowerUsers: ref.power_user_stats.Available,
                    availableStorage: ref.storage_stats.Available,
                    customers: planData.map(
                        (customer: object) => Object.keys(customer)[0]
                    ),
                };
            })
        );
    ...
    async updateCustomerPowerUsers(
        ...
        if (customer.powerUsers.available <= 0)
            throw new Error(
                "No available licenses on customers reseller plan."
            );
        if (
            numOfUsers < customer.powerUsers.used &&
            this._config.forceLicenseChange !== true
        ) {
        ...
        const authCookie = await this._authenticate();
        const res = await this._egnyteRequest(
    ...
    async updateCustomerStorage(
        ...
        const authCookie = await this._authenticate();
        const response = await this._egnyteRequest(
