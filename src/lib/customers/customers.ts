...
export class Customers extends Base {
    ...
    async getAllCustomers(): Promise<EgnyteCustomer[]> {
        ...
        for (const planId of planIds) {
            ...
            for (const customer of usageStats) {
                const [customerEgnyteId, ref] = Object.entries(customer)[0] as [
                    string,
                    UsageStats,
                ];
                ...
                for (const k in ref.feature_stats) {
                    ...
                    obj.features.totalStandardUserPacks = Math.ceil(
                        (ref.feature_stats.additional_su -
                            ref.feature_stats.total_power_users) /
                            5,
                    );
                }
    ...
    async updateCustomerPowerUsers(
        ...
        if (
            ...
            this.config.forceLicenseChange !== true
        ) {
            return {
                result: "NO_CHANGE",
        ...
        if (autoAddToPool) {
            ...
            if (usersNeeded > availablePowerUsers) {
                const licensesToAdd =
                    Math.ceil((usersNeeded - availablePowerUsers) / 5) * 5;
                const updatedLicenesTotal = licensesToAdd + totalPowerUsers;

                await this.plans.UpdatePowerUserLicensing(
                    planId,
                    updatedLicenesTotal,
                );
            }
     ...
     */
    async getCustomerProtectPlanUsage(
        egnyteTenantId: string,
